...
public final class PlayIntegritySpoofService {
    ...
    private volatile boolean mSpoofNetflix = false;
    private volatile boolean mSpoofSnapchat = false;
    ...
    private void loadConfigInternal() {
        ...
        try {
            ...
            String spoofNetflix = am.getSpoofPifSpoofNetflix();
            String spoofSnapchat = am.getSpoofPifSpoofSnapchat();
            mSpoofPhotos = spoofPhotos == null || "1".equals(spoofPhotos)
                            || "true".equalsIgnoreCase(spoofPhotos);
            mSpoofNetflix = spoofNetflix != null && ("1".equals(spoofNetflix)
                            || "true".equalsIgnoreCase(spoofNetflix));
            mSpoofSnapchat = spoofSnapchat != null && ("1".equals(spoofSnapchat)
                            || "true".equalsIgnoreCase(spoofSnapchat));
        } catch (Throwable e) {
    ...
    public boolean shouldSpoof(String processName) {
        ...
        if (SNAPCHAT_PACKAGE.equals(processName)) {
            return mSpoofSnapchat;
        }
        if (NETFLIX_PACKAGE.equals(processName)) {
            return mSpoofNetflix;
        }
        return false;
    }
    ...
    public void spoofBuildFields(String processName) {
        ...
        boolean isSnapchat = SNAPCHAT_PACKAGE.equals(processName);
        boolean isNetflix = NETFLIX_PACKAGE.equals(processName);

        if (!isDroidGuard && !isVending && !isSnapchat && !isNetflix) return;

        if (isSnapchat) {
            for (Map.Entry<String, Object> entry : PIXEL_XL_PROPS.entrySet()) {
                spoofField(entry.getKey(), String.valueOf(entry.getValue()), "SNAP");
            }
            return;
        }

        if (isVending || isNetflix) {
            if (!mSpoofVendingBuild) {
                if (mVerboseLogs > 0) Log.d(TAG, "Vending/Netflix build spoofing disabled");
                return;
            }
            for (Map.Entry<String, String> entry : mBuildFields.entrySet()) {
                if ("SDK_INT".equals(entry.getKey())) continue;
                spoofField(entry.getKey(), entry.getValue(), isVending ? "PS" : "NFLX");
            }
            return;
        }

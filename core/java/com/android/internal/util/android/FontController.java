...
public class FontController {
    ...
    private static final String CUSTOM_FONT_NAME_SETTING = "custom_font_name";
    ...
    private static final Set<String> LEGACY_OVERRIDE_FONTS = new HashSet<>(Arrays.asList(
            "google", "sans-serif", "gsf-"
    ));

    private static final Set<String> LEGACY_SYS_OVERRIDE_FONTS = new HashSet<>(Arrays.asList(
    ...
    public static boolean isCustomFontPickerActive() {
        return get().isCustomFontPickerActiveInternal();
    }

    public static boolean shouldBypassVariableFont() {
        return get().shouldBypassVariableFontInternal();
    }
    ...
    private Typeface getOverrideTypefaceInternal(String fontToOverride) {
        ...
        final boolean bypassVariableFont = shouldBypassVariableFontInternal();
        ...
        if (!bypassVariableFont
                && fontToOverride.contains("variable")
                && currentFont.contains(DEFAULT_FONT_ROOT)) {
            logger("Default font active, passing through variable family: " + fontToOverride);
            return null;
        }

        final Set<String> overrideFonts = bypassVariableFont
                ? LEGACY_OVERRIDE_FONTS : OVERRIDE_FONTS;
        final Set<String> sysOverrideFonts = bypassVariableFont
                ? LEGACY_SYS_OVERRIDE_FONTS : SYS_OVERRIDE_FONTS;

        boolean override = overrideFonts.stream().anyMatch(fontToOverride::contains)
            || (isSysPkg && sysOverrideFonts.stream().anyMatch(fontToOverride::contains));
        if (!override) {
        ...
        if (bypassVariableFont) {
            return TypefaceFactory.createLegacy(fontToOverride, currentFont, adjustment);
        }
        Typeface resolvedBase = resolveDefaultTypeface();
        return TypefaceFactory.create(fontToOverride, resolvedBase, adjustment);
    }

    private boolean isCustomFontPickerActiveInternal() {
        try {
            ...
            String customFontName = Settings.Secure.getStringForUser(
                    resolver,
                    CUSTOM_FONT_NAME_SETTING,
                    UserHandle.myUserId());
            return !TextUtils.isEmpty(customFontName);
        } catch (Exception e) {
            logger("isCustomFontPickerActiveInternal failed: " + e.getMessage());
            return false;
        }
    }

    private boolean shouldBypassVariableFontInternal() {
        if (isCustomFontPickerActiveInternal()) {
            return true;
        }
        if (sResources == null) {
            return false;
        }
        return !DEFAULT_FONT_ROOT.equals(getCurrentFont());
    }
    ...
    private static class TypefaceFactory {
        ...
        public static Typeface createLegacy(
                String fontToOverride, String currentFont, int fontWeightAdjustment) {
            int weight = resolveWeightByName(fontToOverride);
            ...
            logger("TypefaceFactory.createLegacy: fontToOverride=" + fontToOverride +
                   ", style=" + style +
                   ", weight=" + weight +
                   ", adj=" + fontWeightAdjustment +
                   ", isItalic=" + isItalic +

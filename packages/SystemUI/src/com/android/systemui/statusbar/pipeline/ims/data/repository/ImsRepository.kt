...
constructor(
    ...
    fun getRepoForSubId(subId: Int): ImsRepository {
        val cached = cache[subId]?.get()
        if (cached != null) return cached

        val pm = context.packageManager
        val hasTelephony = pm.hasSystemFeature(PackageManager.FEATURE_TELEPHONY)
        val subscriptionManager = context.getSystemService(SubscriptionManager::class.java)
        val hasActiveSubs = subscriptionManager != null && subscriptionManager.activeSubscriptionInfoCount > 0
        val hasValidSub = SubscriptionManager.isValidSubscriptionId(subId)

        if (!hasTelephony || !hasValidSub || !hasActiveSubs) {
            return NoOpImsRepository()
        }
...
class NoOpImsRepository : ImsRepository {
    override val isVoLteAvailable: StateFlow<Boolean> = kotlinx.coroutines.flow.MutableStateFlow(false)
    override val isVoWifiAvailable: StateFlow<Boolean> = kotlinx.coroutines.flow.MutableStateFlow(false)
    override val isVoNrAvailable: StateFlow<Boolean> = kotlinx.coroutines.flow.MutableStateFlow(false)
}
    ...
    private val imsAvailability: StateFlow<ImsAvailabilityState> =
        callbackFlow {
                ...
                try {
                    ...
                    if (e.code == ImsException.CODE_ERROR_SERVICE_UNAVAILABLE) {
                        throw e
                    } else if (e.code != ImsException.CODE_ERROR_UNSUPPORTED_OPERATION &&
                        e.code != ImsException.CODE_ERROR_INVALID_SUBSCRIPTION) {
                        throw e
                    }
            ...
            .retryWhen { cause, attempt ->
                if (cause is ImsException && cause.code == ImsException.CODE_ERROR_SERVICE_UNAVAILABLE) {
                    Log.w(
                        TAG,

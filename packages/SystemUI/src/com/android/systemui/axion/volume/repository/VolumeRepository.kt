...
interface AxionVolumeRepository {
    ...
    val showVolumePercentageFlow: Flow<Boolean>
...
class AxionVolumeRepositoryImpl @Inject constructor(
    ...
    override val inCallFlow: Flow<Boolean> = callbackFlow {
        ...
        var isTelephonyCallActive = telephonyManager.callState != TelephonyManager.CALL_STATE_IDLE
        var isAudioCallActive = audioManager.mode == AudioManager.MODE_IN_CALL ||
                audioManager.mode == AudioManager.MODE_IN_COMMUNICATION

        fun updateInCall() {
            trySend(isTelephonyCallActive || isAudioCallActive)
        }

        val telephonyCallback = object : TelephonyCallback(), TelephonyCallback.CallStateListener {
            override fun onCallStateChanged(state: Int) {
                isTelephonyCallActive = state != TelephonyManager.CALL_STATE_IDLE
                updateInCall()
            }
        }
        
        val modeListener = AudioManager.OnModeChangedListener { mode ->
            isAudioCallActive = mode == AudioManager.MODE_IN_CALL ||
    ...
    override val showVolumePercentageFlow: Flow<Boolean> = settingsFlow(
        uri = Settings.System.getUriFor(Settings.System.SHOW_VOLUME_PERCENTAGE),
        getValue = ::isVolumePercentageEnabled
    )
            ...
            .mapNotNull { appVolume ->
                try {
                    ...
                    if ((appInfo.flags and (ApplicationInfo.FLAG_SYSTEM or ApplicationInfo.FLAG_UPDATED_SYSTEM_APP)) != 0) {
                        if (packageManager.getLaunchIntentForPackage(appVolume.packageName) == null) {
    ...
    private fun isVolumePercentageEnabled(): Boolean =

...
public class PreferredNetworkTile extends QSTileImpl<State> {
    ...
    protected void handleUpdateState(State state, Object arg) {
        state.label = mContext.getResources().getString(R.string.qs_preferred_network_label);
        if (mIcon == null) {
            mIcon = maybeLoadResourceIcon(R.drawable.ic_qs_preferred_network);
        }
        state.icon = mIcon;
        final int mode = getPreferredNetworkMode();
        final int newMode = TelephonyManagerConstants.getTargetMode(mode);
    ...
    public CharSequence getTileLabel() {
        return mContext.getString(R.string.qs_preferred_network_label);
    }

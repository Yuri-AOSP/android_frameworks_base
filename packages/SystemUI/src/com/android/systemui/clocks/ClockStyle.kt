...
class ClockStyle @JvmOverloads constructor(
    ...
    private var clockGradientEnabled = false
    private var gradientColorMode = 0
    ...
    override fun onAttachedToWindow() {
        ...
        tunerService.addTunable(
            ...
            GRADIENT_COLOR_MODE_KEY,
            GRADIENT_START_COLOR_KEY,
            GRADIENT_END_COLOR_KEY,
        )
    ...
    override fun onTuningChanged(key: String?, newValue: String?) {
        when (key) {
            ...
            CLOCK_GRADIENT_ENABLED_KEY -> {
                clockGradientEnabled = TunerService.parseInteger(newValue, 0) != 0
                applyClockColors()
            }
            GRADIENT_COLOR_MODE_KEY -> {
                gradientColorMode = TunerService.parseInteger(newValue, 0)
                if (clockGradientEnabled) applyClockColors()
            }
            GRADIENT_START_COLOR_KEY -> {
                gradientStartColor = TunerService.parseInteger(newValue, 0)
                if (clockGradientEnabled) applyClockColors()
            }
            GRADIENT_END_COLOR_KEY -> {
                gradientEndColor = TunerService.parseInteger(newValue, 0)
                if (clockGradientEnabled) applyClockColors()
            }
        }
    }
    ...
    private fun resolveGradientColors(): Pair<Int, Int> {
        val primary = context.getColor(com.android.internal.R.color.materialColorPrimary)
        val secondary = context.getColor(com.android.internal.R.color.materialColorSecondary)

        if (gradientColorMode == 1) {
            val startColor = if (gradientStartColor != 0) gradientStartColor else primary
            val endColor = if (gradientEndColor != 0) gradientEndColor else secondary
    ...
    private fun applyGradient(tv: TextView, startColor: Int, endColor: Int) {
        val shader = LinearGradient(
            0f, 0f, 0f, tv.textSize,
            startColor, endColor,
    ...
    private fun applyClockColors() {
        ...
        val forceWhiteInAod = isDozing &&
            (clockGradientEnabled || colorMode == COLOR_MODE_ACCENT || colorMode == COLOR_MODE_CUSTOM)

        val (gradStart, gradEnd) = if (clockGradientEnabled && !isDozing) {
        ...
        for (i in textClocks.indices) {
            ...
            if (clockGradientEnabled && !isDozing) {
                if (isWhiteOriginal) {
                    applyGradient(tc, gradStart, gradEnd)
                } else {
                    tc.paint.shader = null
        ...
        for (i in styledTextViews.indices) {
            ...
            if (clockGradientEnabled && !isDozing) {
                if (isWhiteOriginal) {
                    applyGradient(tv, gradStart, gradEnd)
                } else {
    ...
    @com.android.internal.annotations.VisibleForTesting
    fun applyClockScaleAfterLayout(view: View) {
        removePendingLayoutListener()
        applyClockScale()
    ...
    private fun isNoColorClock(style: Int): Boolean {
        return style in CLOCK_LAYOUTS.indices && NO_COLOR_CLOCKS.contains(CLOCK_LAYOUTS[style])
    }
    ...
    companion object {
        ...
        @JvmField val CLOCK_GRADIENT_ENABLED_KEY: String = "system:" + Settings.System.LOCK_SCREEN_CUSTOM_CLOCK_GRADIENT
        @JvmField val GRADIENT_COLOR_MODE_KEY: String = "system:" + Settings.System.CUSTOM_GRADIENT_COLOR_MODE
        @JvmField val GRADIENT_START_COLOR_KEY: String = "system:" + Settings.System.CUSTOM_GRADIENT_START_COLOR
        @JvmField val GRADIENT_END_COLOR_KEY: String = "system:" + Settings.System.CUSTOM_GRADIENT_END_COLOR
